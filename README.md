...
## The delisted state is why this needs a listener

The strongest argument for capturing this store is not that actors die. It is that
**an actor can be removed from the store while still existing**, and only the store
listing records it.

Probing the 15 actors that vanished between the 2026-08-18 archive memento and a
51,603-actor sweep on 2026-09-24:

| state | n | what `/v2/acts/<id>` says |
| --- | ---: | --- |
| deleted | 8 | `404` — gone everywhere |
| **delisted, alive** | **7** | **`200` — the actor is fine; the store just stopped listing it** |

`snipercoder/bulk-linkedin-email-finder` had **472 users a month** and returns `200`
today. Ask the per-actor endpoint and nothing is wrong. Ask the store and it is gone.
**No endpoint records the transition**, so the delisting exists only in the difference
between two listings — which is the definition of the thing this fleet captures.

**And the departures are concentrated.** `LEAD_GENERATION` is 80% of them against
33.9% of the store — **2.36x, p=0.0003**. `AUTOMATION` (1.34x, p=0.17),
`DEVELOPER_TOOLS` (1.22x, p=0.37) and `SOCIAL_MEDIA` (1.64x, p=0.22) are not
significant, so this is lead generation specifically rather than scraping generally.
One owner lost three actors and still lists three, so enforcement lands on the actor,
not the account.

Read it as directional: **n=15 deaths**, actors carry several categories so the four
tests are not independent, and there is exactly **one** prior observation in existence.
That is the whole point — the second one arrives on the next scheduled run.

## What is public and what is not

**Crawling this API is explicitly permitted. Redistributing its records is not** —
no licence appears in the payload and Apify's terms have not been read for a grant
(see [LICENSE-DATA](LICENSE-DATA)). Publishing 34 MB of store records would be
redistribution, so the repository is split along that line:

| | where | what it holds |
| --- | --- | --- |
| **raw records** | **R2** (`storage: object`), gitignored | full store records: username, name, description, pricing |
| **per-actor rows** | `derived/observations/`, gitignored | entity_id, username, usage — their data in tabular form |
| **manifest** | git | urls, hashes, byte counts — transport only, no publisher content |
| **`public/`** | git | **aggregate only.** Bands, Lorenz points, per-partition totals, field volatility. **No actor identity of any kind** |

**That is ~6 KB a week in git against 51 MB.** `public/stats-<date>.json` is what the
figures are drawn from, and it was already identity-free before this split existed —
the charts never needed a username to make their point. Verified by probing the file
for `username`, `userFullName`, `description` and actor slugs: none present.

`public/volatility-<date>.json` is a **frozen one-off**, not a weekly product. Payload
share cannot be recomputed from this repo's own captures because `project_drop`
already removes the fields it measures, and the only before-picture is a single
Internet Archive memento. Both sides of that measurement are unprojected, because
comparing a full record to a trimmed one measures the trim rather than the publisher.

Running the capture needs R2 credentials (`R2_ACCOUNT_ID`, `R2_ACCESS_KEY_ID`,
`R2_SECRET_ACCESS_KEY`, `R2_BUCKET_NAME`, `WSS_OBJECT_PREFIX`). Without them the
object config resolves empty and the job cannot write a byte.
...
| Q3 | When an actor disappears, was its usage already falling — or did it vanish healthy? | **first evidence: it vanishes healthy, and there are three states not two.** 15 of the 949 actors in the 2026-08-18 memento are absent from a 51,603-actor sweep 37 days later. Probing `/v2/acts/<id>` for each: **8 return 404 (deleted), 7 return 200 — still alive, but delisted from the store.** None had zero users; median 32/month, max 472. **LEAD_GENERATION is 80% of departures against 33.9% of the store — 2.36x, p=0.0003**; no other category is significant. One owner lost 3 and kept 3, so it is per-actor, not per-account. n=15, categories overlap, directional not settled |
